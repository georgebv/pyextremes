...
from pyextremes.models import Lmoments, get_model, Distribution
...
@pytest.fixture(scope="function")
def lmoments_model(extremes) -> Lmoments:
    return get_model(
        model="Lmoments",
        extremes=extremes,
...
class TestLmoments:
    def test_model(self, extremes, lmoments_model):
        # Test extremes attribute
        assert np.all(lmoments_model.extremes.index == extremes.index)
        assert np.allclose(lmoments_model.extremes.values, extremes.values)

        # Test fit_parameters attribute
        assert lmoments_model.distribution.name == "genextreme"
        assert len(lmoments_model.fit_parameters) == 3
        for key, value in {"loc": 10, "scale": 2}.items():
            assert key in lmoments_model.fit_parameters
            assert np.isclose(lmoments_model.fit_parameters[key], value, rtol=0, atol=0.1)

        # Test trace attribute
        with pytest.raises(TypeError, match=r"trace property is not"):
            lmoments_model.trace

        # Test return_value_cache attribute
        assert isinstance(lmoments_model.return_value_cache, dict)
        assert len(lmoments_model.return_value_cache) == 0

        # Test name attribute
        assert lmoments_model.name == "Lmoments"

        # Test fit_parameter_cache attribute
        assert isinstance(lmoments_model.fit_parameter_cache, list)
        assert len(lmoments_model.fit_parameter_cache) == 0

    def test_fit(self, lmoments_model):
        with pytest.raises(TypeError, match=r"unrecognized arguments.*bad_argument"):
            lmoments_model.fit(bad_argument=1)

    def test_loglikelihood(self, extremes, lmoments_model):
        assert np.allclose(
            np.sum(
                scipy.stats.genextreme.logpdf(
                    x=extremes.values,
                    **lmoments_model.fit_parameters,
                    **lmoments_model.distribution._fixed_parameters,
                )
            ),
            lmoments_model.loglikelihood,
            rtol=0,
            atol=0.01,
        )

    def test_aic(self, extremes, lmoments_model):
        k = 2
        n = len(extremes)
        aic = 2 * k - 2 * lmoments_model.loglikelihood
        correction = (2 * k**2 + 2 * k) / (n - k - 1)
        assert np.isclose(lmoments_model.AIC, aic + correction, rtol=0, atol=3)

    @pytest.mark.parametrize("prop", ["pdf", "cdf", "ppf", "isf"])
    def test_properties(self, prop, lmoments_model):
        assert np.isclose(
            getattr(lmoments_model, prop)(x=0.1),
            getattr(scipy.stats.genextreme, prop)(0.1, c=0, loc=10, scale=2),
            rtol=0.1,
            atol=0.2,
        )

    def test_get_return_value(self, lmoments_model):
        with pytest.raises(TypeError, match=r"unrecognized arguments.*bad_argument"):
            lmoments_model.get_return_value(exceedance_probability=0.1, bad_argument=1)
        with pytest.raises(ValueError, match=r"invalid shape.*exceedance_probability"):
            lmoments_model.get_return_value(exceedance_probability=[[1, 2, 3], [1, 2, 3]])
        with pytest.raises(
            ValueError, match=r"invalid value.*n_samples.*must be positive"
        ):
            lmoments_model.get_return_value(exceedance_probability=0.1, n_samples=-1)

        # Test scalar, no alpha
        rv, cil, ciu = lmoments_model.get_return_value(exceedance_probability=0.1)
        assert np.isclose(
            rv, scipy.stats.genextreme.isf(0.1, c=0, loc=10, scale=2), rtol=0, atol=0.1
        )
        assert np.isnan(cil) and np.isnan(ciu)
        assert len(lmoments_model.return_value_cache) == 1
        assert len(lmoments_model.fit_parameter_cache) == 0
        assert len(np.unique(lmoments_model.fit_parameter_cache, axis=0)) == len(
            lmoments_model.fit_parameter_cache
        )
        seed_cahe_size = 0
        assert len(lmoments_model.seed_cache) == seed_cahe_size

        # Test scalar, with alpha
        rv, cil, ciu = lmoments_model.get_return_value(
        ...
        assert len(lmoments_model.return_value_cache) == 2
        assert len(lmoments_model.fit_parameter_cache) == 100
        assert len(np.unique(lmoments_model.fit_parameter_cache, axis=0)) == len(
            lmoments_model.fit_parameter_cache
        )
        ...
        assert len(lmoments_model.return_value_cache) == 3
        assert len(lmoments_model.fit_parameter_cache) == 100
        assert len(np.unique(lmoments_model.fit_parameter_cache, axis=0)) == len(
            lmoments_model.fit_parameter_cache
        )
        ...
        assert not (np.isnan(cil) or np.isnan(ciu))
        assert len(lmoments_model.return_value_cache) == 4
        assert len(np.unique(lmoments_model.fit_parameter_cache, axis=0)) == len(
            lmoments_model.fit_parameter_cache
        )
        ...
        assert not (np.isnan(cil) or np.isnan(ciu))
        assert len(lmoments_model.return_value_cache) == 5
        assert len(lmoments_model.fit_parameter_cache) == 201
        assert len(np.unique(lmoments_model.fit_parameter_cache, axis=0)) == len(
            lmoments_model.fit_parameter_cache
        )
        seed_cahe_size += min(2, os.cpu_count())
    ...
    def test_repr(self, lmoments_model):
        repr_value = str(lmoments_model)
    ...
    def test_lmoments_distributions(
